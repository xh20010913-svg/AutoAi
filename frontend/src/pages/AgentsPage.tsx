...
function AgentCard({ agent, t }: { agent: Agent; t: (key: string) => string }) {
  const [hovered, setHovered] = useState(false)
  const Icon = agent.icon
  return (
    <div className="group relative bg-card p-4 transition-colors hover:border-primary pixel-border" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <div className={cn("h-2 w-2 rounded-full", agent.status === "running" ? "bg-emerald-500 animate-pulse" : agent.status === "error" ? "bg-red-500 animate-pulse" : "bg-zinc-500")} />
          </div>
      ...
      <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span>{agent.completedTasks} {t("agents.tasksDone")}</span>
      </div>
      ...
      {hovered && (
        <div className="absolute inset-x-0 bottom-0 flex justify-end gap-1 bg-gradient-to-t from-card p-2">
          <button className="p-1.5 text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors border border-transparent hover:border-border" title={t("agents.start")}><Play className="h-3.5 w-3.5" /></button>
          <button className="p-1.5 text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors border border-transparent hover:border-border" title={t("agents.stop")}><Square className="h-3.5 w-3.5" /></button>
          <button className="p-1.5 text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors border border-transparent hover:border-border" title={t("agents.settings")}><Settings className="h-3.5 w-3.5" /></button>
        </div>
      )}
    </div>
  )
    ...
    <div>
      <div className="mb-6">
        ...
        <div className="flex items-center gap-4 text-sm text-muted-foreground">
          <span>{t("agents.total")}: {mockAgents.length}</span>
          <span className="flex items-center gap-1.5"><span className="h-2 w-2 bg-emerald-500 animate-pulse" />{t("agents.running")}: {running}</span>
          {errored > 0 && (<span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-red-500 animate-pulse" />{t("agents.error")}: {errored}</span>)}
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {mockAgents.map((agent) => (<AgentCard key={agent.id} agent={agent} t={t} />))}
      </div>
    </div>
  )
}
